...
Distribution fairness based on your current weight settings. Score of 100 = perfect fairness, lower scores indicate imbalance across reps.

### Balanced Fairness Score
Distribution fairness using equal weights (33% ARR, 33% Accounts, 33% Risk). Provides an unbiased fairness benchmark.

### Coefficient of Variation (CV%)
Statistical measure of distribution balance. Lower is better. CV% < 10% = Excellent, 10-15% = Good, 15-20% = Fair, >20% = Poor distribution.

### Segment Fairness
Fairness score for Enterprise or Mid-Market segment only. Helps identify if one segment is better balanced than the other.

### Average Fairness
Overall fairness across both segments, weighted by number of reps in each segment.
...
### What is the Sensitivity Chart?
Shows how fairness and segment balance change as you adjust the employee count threshold. Helps identify the optimal threshold for your goals.
...
Shows accounts that changed reps between original and new allocation. Helps identify potential customer disruption.

### KPI Improvement Panel
Before/After comparison of CV% for ARR, Accounts, and Risk. Green = improvement, red = worse distribution.
